...
.. important::
   If the subscription is on a monthly subscription plan, the database will automatically update to
   reflect the added user. If it is on a yearly or multi-year subscription, an expiration banner
   will appear in the database. The Odoo customer can create the upsell quote themselves by clicking
   the banner to update the subscription or a Customer Service Manager can help resolve the issue.
